...
class Convert():
    ...
    def convert_orders_to_date_format(self):
        ...
        self.dataframes["ODL_ORDER"]['creation_date_order'] = pd.to_datetime(self.dataframes["ODL_ORDER"]['creation_date_order'], errors='coerce') #This function take time !!!
    ...
    def rename_columns(self):
        ...
        self.dataframes["ODL_ORDER_ITEM"].rename(columns={"amount":"amount_of_item"},inplace=True)
        self.dataframes["ODL_ORDER_ITEM"].rename(columns={"id":"order_item_id"},inplace=True)
        self.dataframes["ODL_ORDER_ITEM"].drop("Column_useless",axis=1,inplace=True)
        self.dataframes["ODL_ORDER"].rename(columns={"creation_date":"creation_date_order"},inplace=True)
        self.dataframes["ODL_ORDER"].rename(columns={"id":"order_id"},inplace=True)
        self.dataframes["ODL_ORDERABLES"].rename(columns={"id":"orderable_id"},inplace=True)
        self.dataframes["ODL_ORDERABLES"].rename(columns={"name":"orderable_name"},inplace=True)
        self.dataframes["ODL_RESTAURANT"].rename(columns={"id":"restaurant_id"},inplace=True)
        self.dataframes["ODL_RESTAURANT"].rename(columns={"name":"restaurant_name"},inplace=True)
        self.dataframes["ODL_RESTAURANT"].rename(columns={"creation_date":"restaurant_creation_date"},inplace=True)
    ...
    def change_unknown_city(self,dataframes) -> pd.DataFrame:
        ...
        self.dataframes["ODL_RESTAURANT"]["city"] = self.dataframes["ODL_RESTAURANT"]["city"].fillna(value="unknown")
            ...
            if city == "unknown":
                ...
                if address is not None:
                    ...
                    if address is not None:
                        ...
                        self.dataframes["ODL_RESTAURANT"].loc[x,"city"] = "San Francisco"   
                    
                    else:

                        self.unknown_street.append(self.dataframes["ODL_RESTAURANT"].loc[x,"street"])
